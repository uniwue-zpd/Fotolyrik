      ...
      <template #header="{ chooseCallback, uploadCallback, clearCallback, files }">
        <div class="flex flex-wrap justify-between items-center flex-1 gap-4">
            <div class="flex gap-2">
                <Button @click="chooseCallback()" label="Auswählen" icon="pi pi-images" rounded-sm outlined severity="secondary"></Button>
                <Button @click="uploadCallback" label="Hochladen" icon="pi pi-cloud-upload" rounded-sm outlined severity="success" :disabled="!files || files.length === 0"></Button>
                <Button @click="clearCallback()" icon="pi pi-times" rounded-sm outlined severity="danger" :disabled="!files || files.length === 0"></Button>
            </div>
